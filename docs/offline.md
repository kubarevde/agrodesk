...
> **Важно:** одного «добавить на экран» недостаточно. Нужен **предварительный online-визит** в «Моя смена» / «Рабочее время» **под своим логином**, чтобы Service Worker, профиль (`agrodesk_user_cache` привязан к JWT `sub`) и справочники попали в кэш. Без этого форма открытия смены офлайн будет пустой, а чужой кэш (EMP001 после входа EMP000) — отброшен.
