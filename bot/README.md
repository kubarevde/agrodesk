...
В Telegram: `/start` → «🟢 Начал работу» → (план на сегодня, если есть) → объект *(или авто «Полевая работа» для полевых типов)* → гео (можно пропустить) → тип работ → *(поле, если `is_field_work`)* → техника → комментарий.
...
| Действие | Кнопка |
|----------|--------|
| Открыть смену | 🟢 Начал работу (с выбором плана агрокалендаря на сегодня) |
| Закрыть смену | 🔴 Закончил работу (описание ≥ 5 символов) |
| Текущая смена | 📊 Мой статус (с гео / Google Maps) |
| Смены по дате | 📅 Смены по дате → Сегодня / Вчера / календарь |
| Рассылки (admin) | 📣 Написать всем / 📣 Написать кто на смене |
...
- Полевая работа: тип с `is_field_work=true` → объект «Полевая работа» + обязательное поле (как в веб).
- Геометка при открытии опциональна (как в веб «Моя смена»).
- Журнал ремонта и смена статусов — только в веб; бот не дублирует этот UI.
